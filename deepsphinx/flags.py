...
def load_flags(mode = 'train'):
    flags = tf.app.flags
    # flags.DEFINE_string('cuda-visible-devices', None, 'List of CUDA Devices to use, example: 0,1.')
    ...
    flags.DEFINE_integer('max-output-len', 20, 'Maximum output length for Beam Search')
    flags.DEFINE_integer('rnn-size', 256, 'Size of LSTM')
    flags.DEFINE_integer('num-layers', 3, 'Number of LSTM Layers for encoding layer')
    flags.DEFINE_integer('num-decoding-layers', 3, 'Number of LSTM Layers for decoding layer. At least two.')
    flags.DEFINE_integer('batch-size', 32, 'Batch size of data')
    flags.DEFINE_integer('max-segment-length', 12, 'Maximum length of a segment')
    ...
    flags.DEFINE_float('learning-rate-decay', 0.999999, 'Learning rate decay')
    flags.DEFINE_float('min-learning-rate', 0.0005, 'Minimum learning rate')
    ...
    flags.DEFINE_integer('use-joint-prob', 1, 'Use joint probability model')
    ...
    flags.DEFINE_bool('inflection-task', False, 'Use inflection task model')
