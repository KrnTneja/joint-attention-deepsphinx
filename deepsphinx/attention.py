...
class BahdanauAttentionCutoff(tf.contrib.seq2seq.BahdanauAttention.__base__):
    ...
    def __call__(self, query, previous_alignments, expansion=1):
        ...
        with tf.variable_scope(None, 'bahdanau_attention', [query]):
            ...
            keys_shape = self._keys.shape
            # print("Keys Shape", keys_shape)
            keys_new_shape = (keys_shape[0].value*expansion, -1,) + (keys_shape[2].value,)
            # print("Keys New Shape", keys_new_shape)
            expanded_keys = tf.reshape(tf.tile(self._keys, [1, 1, expansion]), 
                keys_new_shape)
            if self._normalize:
                # normed_v = g * v / ||v||
                normed_v = self.g * self.v * tf.rsqrt(
                    tf.reduce_sum(tf.square(self.v)))
                # keys = tf.reshape(tf.tile(keys, [1, 1, keys_expansion]), 
                #     (processed_query_shape[0], -1, processed_query_shape[-1]))
                score = tf.reduce_sum(
                    normed_v * tf.tanh(expanded_keys + processed_query + self.b), [2])
            else:
                if FLAGS.use_conv_feat_att:
                    score = tf.reduce_sum(self.v * tf.tanh(expanded_keys + processed_query + conv_feat),
                                      [2])
                else:
                    score = tf.reduce_sum(self.v * tf.tanh(expanded_keys + processed_query),
                                      [2])

        # print("score", score)
        # print("previous_alignments", previous_alignments)
        alignments = self._probability_fn(score, previous_alignments, expansion)
        # alignments = tf.Print(alignments, [my_tf_round(alignments, decimals=3)], message="Alpha_i's", summarize=1000)
        # tf.summary.histogram("Alignments (alpha_i's)", alignments)
        return alignments
